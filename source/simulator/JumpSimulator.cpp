...
void JumpSimulator::generateTakeoffRating()
{
    ...
    double ratingMultiplier = 0.8275
    ...
    ratingMultiplier = 0.1725 + 0.1 * hill->getLevelOfCharacteristic("takeoff-form-effect");
...
void JumpSimulator::generateFlightRating()
{
    ...
    double ratingMultiplier = 0.8175
    ...
    ratingMultiplier = 0.1825 + 0.1 * hill->getLevelOfCharacteristic("flight-form-effect");
...
void JumpSimulator::generateWindEffects()
{
    ...
    for (const auto &wind : getWinds()) {
        if (wind.getDirection() <= 90 || wind.getDirection() >= 270) //Wiatr przedni
        {
            ...
            qDebug() << "WIATR " << i + 1 << ": " << change << "change"
                     << " DIRECTION: " << wind.getDirection() << " (WIATR PRZEDNI)";
        } else //Wiatr tylny
        {
            ...
            qDebug() << "WIATR " << i + 1 << ": " << change << "change"
                     << " DIRECTION: " << wind.getDirection() << " (WIATR TYLNI)";
        }
        ...
        if (i != getWinds().count()) {
            qDebug() << "distance: " << jumpData.getDistance()
            ...
            if (jumpData.getDistance() < (i + 1) * getWindSegmentDistance()) {
                ...
                double percent = 1;
                if (i > 0)
                    percent = ((jumpData.getDistance())
                               - (double(getWindSegmentDistance() * (i + 0))))
...
void JumpSimulator::generateLanding()
{
    ...
    double landingRating = MyRandom::lognormalDistributionRandom(
        (0.5 - ((jumperSkills->getLandingStyle() - 10) / 8.5)),
        (0.3 + hill->getLandingImbalanceChangeByHillProfile(jumpData.distance)));
    ...
    else if (jumpData.landing.getRating() > manipulator->getLandingRatingRange().second
             && manipulator->getLandingRatingRange().second > (-1))
...
void JumpSimulator::generateJudges()
{
    if (competitionRules->getHasJudgesPoints() == true) {
        ...
        simulationData->judgesRating -= jumpData.landing.getRating() / 1.675;
        ...
        for (auto &jg : jumpData.judges) {
            ...
            switch (jumpData.landing.getType()) {
            case Landing::TelemarkLanding:
                random = MyRandom::normalDistributionRandom(0, 0.2);
                break;
            case Landing::BothLegsLanding:
                random = MyRandom::normalDistributionRandom(0, 0.4);
                break;
            case Landing::SupportLanding:
                random = MyRandom::normalDistributionRandom(0, 0.7);
                break;
            case Landing::Fall:
                random = MyRandom::normalDistributionRandom(0, 0.625);
                break;
            }
            if (randomType == 0)
                jg -= random;
...
double JumpSimulator::getRandomForJumpSimulation(short parameter, Jumper *jumper)
{
    ...
    case JumpSimulator::TakeoffRating: {
        ...
        double devAddition = -((dev + double((double(skills->getJumpsEquality()) / 1.55))) / dev)
                             - 1;
